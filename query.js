...
jsonQuery.query = function (/*String*/ query, /*Object?*/ obj) {
	...
	query = query.replace(/([^<>=]=)([^=>])/g, "$1=$2") // change the equals to comparisons except operators ==, <=, >=, => (lambda)
		.replace(/(?<=\()((.*?=>.*?)|function\s?\(.*?\)\s?{.*?})(?=\))/g, t => "~" + (lambdas.push(t) - 1)) // lambda and normal function support, further rules will not be processed between the parenthesis
		...
		.replace(/~([0-9]+)/g, (t, a) => lambdas[a]) // restore lambdas
		.replace(/`([0-9]+|\])/g, (t, a) => a == ']' ? ']' : str[a]); //restore the strings
	
	query = prefix + (query.charAt(0) == '$' ? "" : "$") + query;		
		
	var args = [].slice.call(arguments);
	
	// skip first two args (query and object)
	var func_args = args.slice(2).map((v, i) => `$${(i + 1)}`).join(', ');	
	func_args = func_args ? '$, ' + func_args : '$'
	
	var func_str = "(function(" + func_args + "){ var $obj = $; return " + query + "; })";
		
	// for logging purposes	
	this._log.query.unshift(query);
	this._log.func.unshift(func_str);
	
	// create a function within this scope (so it can use expand and slice)
	var executor = eval(func_str);
